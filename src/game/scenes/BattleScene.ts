...
export class BattleScene extends Phaser.Scene {
  ...
  private weaponInfoGfx!: Phaser.GameObjects.Graphics  // 主武器信息图形（canvas内）
  private weaponInfoTexts: Phaser.GameObjects.Text[] = []  // 主武器信息文本对象
  ...
  create() {
    ...
    this.weaponInfoGfx.setDepth(1000) // 确保在最上层
  ...
  private drawDamageStats() {
    ...
    this.damageStatsTexts.push(title)
    y += lineHeight + 2
    
    // 伤害项
    for (const [source, damage] of sorted) {
      ...
      this.damageStatsTexts.push(textObj)
